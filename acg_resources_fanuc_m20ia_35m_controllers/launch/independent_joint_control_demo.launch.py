...
def generate_launch_description():
    ...
    # Load RViz configuration with planning plugins
    rviz_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([FindPackageShare("acg_resources_fanuc_m20ia_35m_moveit_config"), "launch/moveit_rviz.launch.py"])
        ),
    )
    ...
    motion_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["fanuc_m20ia_35m_PID_controller", "--controller-manager", "/controller_manager"],
    )
    ...
    delay_ignition_after_robot_state_pub_node = RegisterEventHandler(
        event_handler=OnProcessStart(
            target_action=robot_state_pub_node,
            on_start=[ignition_spawn_entity]
        )
    )    

    nodes = [
        delay_ignition_after_robot_state_pub_node,
        delay_controllers_after_ignition_spawn,
        delay_joint_state_broadcaster_spawner_after_ignition_spawn,
        delay_rviz_after_joint_state_broadcaster_spawner,
    ]

    return LaunchDescription(declared_arguments + launches + nodes + [group_action])